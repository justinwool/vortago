...
if ($access_token != '') {
//instagram
$instID_url = wp_remote_get('https://api.instagram.com/v1/users/search?q='.$instagram.'&access_token=30893233.5ccd4be.b5c20ef113624b6f992e367eaa125d9d');
if(is_wp_error($instID_url)) { return; }
$instID_json = json_decode( $instID_url['body'], true );
$instID = $instID_json['data'][0]['id'];
$data = wp_remote_get('https://api.instagram.com/v1/users/'.$instID.'/?access_token=30893233.5ccd4be.b5c20ef113624b6f992e367eaa125d9d');
    if (!is_wp_error($data)) {
        $json = json_decode( $data['body'], true );
