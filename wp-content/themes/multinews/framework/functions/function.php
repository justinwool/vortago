		...
		function mom_post_image_full($size = 'thumbnail', $hd = '', $alt = '', $id = ''){
			if (mom_post_image($size, $id) != '') {
				...
				if( has_post_thumbnail() || mom_option('theme_thumb') != 1) {
					...
					if($image['url'] != ''){
						$output = '<img itemprop="image" src="'.$image['url'].'" data-hidpi="'.mom_post_image($hd, $id).'" alt="'.$alt.'" width="'.$image['width'].'" height="'.$image['height'].'">';
					} else {
						$output = '<img itemprop="image" src="'.mom_post_image($size, $id).'" data-hidpi="'.mom_post_image($hd, $id).'" alt="'.$alt.'" width="'.$image['width'].'" height="'.$image['height'].'" style="max-height:'.$h.'px;">';
					}
				} else {
					$output = '<img itemprop="image" src="'.mom_post_image($size, $id).'" data-hidpi="'.mom_post_image($hd, $id).'" alt="'.$alt.'" width="'.$w.'" height="'.$h.'">';
				}
