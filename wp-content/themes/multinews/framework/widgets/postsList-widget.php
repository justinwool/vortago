...
?>
		<ul class="post-list jwool">
			...
			<?php } elseif($orderby == 'Recent') { ?>
				...
				$catsi = implode(',', $cats);
				?>
				<?php $query = new WP_Query( array( 'post_type' => array('post','appearance','program'), 'post_status' => 'publish', 'cat' => $catsi, 'ignore_sticky_posts' => 1, 'posts_per_page' => $count, 'no_found_rows' => true, 'cache_results' => false, 'post__not_in' => $do_unique_posts ) ); ?>
				<?php } elseif ($display == 'tags') { ?>
				...
				<?php } else { ?>
				<?php $query = new WP_Query( array( 'post_type' => array('post','appearance','program'), 'post_status' => 'publish', 'ignore_sticky_posts' => 1, 'posts_per_page' => $count, 'no_found_rows' => true, 'cache_results' => false, 'post__not_in' => $do_unique_posts ) ); ?>
				<?php } ?>
			<?php } ?>
			...
			<li <?php post_class(); ?> itemscope="" itemtype="http://schema.org/Article">
				<h2 itemprop="name">
                    <a itemprop="url" class="entry-title" href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
               </h2>
