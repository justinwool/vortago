...
function ads_extra_cols($columns) {
	...
	//$columns['views'] = __('Views', 'framework');
    $columns['clicks'] = __('Clicks', 'framework');
    return $columns;
}
 ...
 ?>
	...
	<script type="text/javascript">
		jQuery(document).ready(function($) {
			jQuery('.ads-rotator-id-<?php echo $rndn; ?> .mom-ads-inner').imagesLoaded( function() {
	...
	<div class="mom-ads-wrap <?php echo $class; ?> <?php echo $c_class; ?>">
	...
	<?php
	$i = 0;
	$ran = isset($ad_setting['ad_rand']) ? $ad_setting['ad_rand'] : '';
	$numb = isset($ad_setting['ad_count']) ? $ad_setting['ad_count'] : '';
	if (!isset($ads['ads'])) {
	...
	foreach ($ads['ads'] as $ad) {
		...
		$url_target = isset($ad['ad_url_target']) ? $ad['ad_url_target'] : '_self';
	...
	<?php if ($output) { ?>
	<div class="mom-ad" data-id="<?php echo $id; ?>" style="width:<?php echo isset($size[0]) ? $size[0] : ''; ?>px; height:<?php echo isset($size[1]) ? $size[1] : ''; ?>px; <?php echo $space; ?>">
		<?php if ($type == 'code') { 
			echo '<div class="ad-code">'.do_shortcode($code).'</div>';
		} else { ?>
			<a href="<?php echo $url; ?>" target="<?php echo $url_target; ?>"><img class="appear" src="<?php echo $img ?>" alt="<?php echo get_the_title($id); ?>" width="<?php echo $size[0]; ?>" height="<?php echo $size[1]; ?>"></a>
		 <?php } ?>
	</div><!--mom ad-->
	<?php } else { ?>
		<div class="mom-ad mom-ad-empty border-box" style="width:<?php echo isset($size[0]) ? $size[0] : ''; ?>px; height:<?php echo isset($size[1]) ? $size[1] : ''; ?>px; line-height:<?php echo $size[1]; ?>px; <?php echo $space; ?>">
