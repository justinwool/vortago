...
$q = <<<MOUT
SELECT *, DATE_FORMAT(appear_dt,"%M %D, %Y") dt2
...
while($row = $result->fetch_assoc()) {
	$myAppearances[] = $row;
	if ($row["show_lat"] && $row["show_long"])
		$markers[] = "['" . $row["show_nm"] . "'," . $row["show_lat"] . "," . $row["show_long"] . "]"; 
}


/*
for ($i=0;$i<=min(2,count($myAppearances)-1);$i++){
	$recentAppearances[] = $myAppearances[count($myAppearances)-$i]["show_nm"];
}
*/

for ($i=count($myAppearances)-1;$i>=0;$i--){
	$recentAppearances[$myAppearances[$i]["show_pk"]] = $myAppearances[$i]["show_nm"];	
	if (count($recentAppearances)>=3) break;
}

if (count($recentAppearances)<=2)
	$myRecentAppearances = implode(" and ",$recentAppearances);
else {
	$lastAppearance = array_pop($recentAppearances);
	$myRecentAppearances = implode(", ",$recentAppearances) . " and " . $lastAppearance;
}
...
?>
											...
											<div class="story-highlights">
													<h4>Highlights</h4>
													<ul>
														<li><a href="#h">Number of Appearances: <?php echo count($myAppearances);?></a></li>
														<li><a href="#h">Earliest: <?php echo $myAppearances[count($myApearances)]["dt2"];?></a></li>
														<li><a href="#h">Most Recent: <?php echo $myAppearances[0]["show_nm"];?></a></li>
													</ul>
											</div>
<!-- end JW -->
...
<p>
<span itemprop="person" itemscope itemtype="http://schema.org/Person"><span itemprop="name"><?=$myInfo["person_nm"];?></span>
<?php if ($myInfo["person_wiki"]<>"") { ?>
	<link itemprop="sameAs" href="<?=$myInfo["person_wiki"];?>"/>
<?php } ?>
</span>
has <?php echo count($myAppearances);?> <?php echo (count($myAppearances)>1) ? "appearances" : "appearance";?> archived on Vortago.  <?=$myInfo["person_nm"];?> has recently appeared on <?=$myRecentAppearances;?>.
</p>

<p>For a complete list of all lectures, interviews, and debates with <?=$myInfo["person_nm"];?>, please see the official Vortago listing below.</p>
